...
function KanbanColumn({ status, tasks }: { status: KanbanStatus; tasks: Task[]; }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 p-2 mb-4">
        <span className={`w-3 h-3 rounded-full flex-shrink-0 ${statusToColor[status]}`}></span>
        <h3 className="font-semibold text-sm sm:text-base uppercase tracking-wider">{status}</h3>
